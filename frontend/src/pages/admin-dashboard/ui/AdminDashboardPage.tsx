...
import type { DashboardResponse } from '@/features/admin-dashboard/model/type';
...
function ApprovalSection({ title, icon, count, children, emptyMessage, isLoading }: ApprovalSectionProps) {
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-md overflow-hidden">
...
function RejectModal({ target, reason, onReasonChange, onConfirm, onCancel, isLoading }: RejectModalProps) {
    ...
    <Dialog open={target !== null} onOpenChange={(v) => { if (!v) onCancel(); }}>
      <DialogContent className="max-w-md rounded-2xl">
        ...
        <div className="flex flex-col gap-3 py-2">
          ...
          <Textarea
            placeholder={reasonRequired ? '반려 사유를 입력하세요.' : '거절 사유를 입력하세요. (선택)'}
            value={reason}
...
function ApprovalsTab() {
  ...
  const handleRejectConfirm = () => {
    ...
    const onSettled = () => { setRejectTarget(null); setRejectReason(''); };
    if (rejectTarget.kind === 'user') {
      rejectUser({ memberId: rejectTarget.id, data: { reason: rejectReason || undefined } }, { onSettled });
...
const AdminDashboardPage = () => {
  ...
  const { data: dashboardData, isLoading: isDashboardLoading } = useAdminDashboardQuery(year, month);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-1.5">
        <div className="flex gap-1">
          <button type="button" className={mainTabCls(activeTab === 'payroll')} onClick={() => setActiveTab('payroll')}>
            급여현황
          </button>
          <button type="button" className={mainTabCls(activeTab === 'approvals')} onClick={() => setActiveTab('approvals')}>
            승인대기
            {totalPendingCount > 0 && (
