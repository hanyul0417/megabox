...
type PayslipTab = 'confirmed' | 'estimated';
...
const payslipTabCls = (active: boolean) =>
  cn(
...
function isConfirmed(p: PayrollData): boolean {
  return !!p.payroll_id && p.payroll_id > 0;
}
...
function PayslipPanel({ employee, open, onClose }: PayslipPanelProps) {
  const [activeTab, setActiveTab] = useState<PayslipTab>('confirmed');

  // 패널이 열릴 때마다 기본 탭 결정
  const confirmed = employee ? isConfirmed(employee) : false;
  const displayTab: PayslipTab = !confirmed && activeTab === 'confirmed' ? 'estimated' : activeTab;
    ...
    >
      <SheetContent side="right" className="w-full sm:w-[420px] flex flex-col p-0 overflow-hidden">
        ...
        <SheetHeader className="px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
          ...
          {/* 탭 토글 */}
          <div className="flex items-center bg-gray-100/80 rounded-xl p-1 gap-0.5 mt-4">
            <button
              type="button"
              disabled={!confirmed}
              onClick={() => setActiveTab('confirmed')}
              className={cn(
                payslipTabCls(displayTab === 'confirmed'),
            ...
            <button
              ...
              className={payslipTabCls(displayTab === 'estimated')}
            >
              예상
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {displayTab === 'confirmed' && !confirmed ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-14 h-14 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center mb-4">
                <DollarSign className="size-6 text-gray-300" />
              </div>
              <p className="font-semibold text-gray-500">미확정 상태입니다</p>
              <p className="text-sm text-gray-400 mt-1">아직 급여가 확정되지 않았습니다.</p>
            </div>
          ) : (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {employees.map((emp) => {
              const confirmed = isConfirmed(emp);
              return (
                <tr
                  key={emp.user_id}
...
function PayrollTab({ year, month }: PayrollTabProps) {
  const [selectedEmployee, setSelectedEmployee] = useState<PayrollData | null>(null);
  const { data: rawPayroll, isLoading } = usePayrollQuery({ year, month });
  ...
  const totalGrossPay = useMemo(
    () => employees.reduce((sum, e) => sum + e.gross_pay, 0),
    [employees],
  );
  const confirmedCount = useMemo(() => employees.filter(isConfirmed).length, [employees]);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KpiCard
          ...
          label="이번달 인건비"
          value={isLoading ? '-' : `${fmt(totalGrossPay)}원`}
          sub="세전 총급여 합계"
        ...
        <KpiCard
          ...
          label="급여 확정 현황"
          value={isLoading ? '-' : `${confirmedCount} / ${totalCount}명 확정`}
          sub={`미확정 ${totalCount - confirmedCount}명`}
          accent="bg-blue-50"
        />
        <KpiCard
          icon={<DollarSign className="size-4 text-amber-500" />}
          label="예상 인건비 (근태기록 기반)"
          value={isLoading ? '-' : `${fmt(totalGrossPay)}원`}
          sub="수당 포함, 확정 전 변경 가능"
          accent="bg-amber-50"
        />
      </div>

      {/* 직원 테이블 */}
      <EmployeeTable employees={employees} isLoading={isLoading} onRowClick={setSelectedEmployee} />
      ...
      >
        {pendingUsers.map((u) => (
          <ApprovalRow
            ...
            onReject={() => openReject({ kind: 'user', id: u.id, name: u.name })}
      ...
      >
        {pendingDayoffs.map((d) => (
          <ApprovalRow
            ...
            onReject={() => openReject({ kind: 'dayoff', id: d.id, name: d.user_name })}
      ...
      >
        {pendingShifts.map((s) => (
          <ApprovalRow
            ...
            onReject={() => openReject({ kind: 'shift', id: s.id, name: s.requester_name })}
    ...
    <div className="flex flex-col gap-5">
      ...
      {activeTab === 'payroll' && (
        <div className="flex flex-col gap-4">
          ...
          <PayrollTab year={year} month={month} />
        </div>
      )}
